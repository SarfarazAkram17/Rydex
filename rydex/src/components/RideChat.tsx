  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white rounded-2xl overflow-hidden border border-zinc-100">
      ...
      >
        ...
        {messages.length > 0 &&
          messages.map((m, i) => {
            const isMine = m.sender.toString() === currentRole;
            ...
            <div className="px-4 pt-3 pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  <Sparkles size={12} className="text-shadow-violet-500" />
                ...
                <div className="flex flex-col gap-1.5">
                  {suggestions.map((s, i) => (
                    <motion.div
                      ...
                      className="text-left text-sm text-zinc-700 bg-zinc-50 hover:bg-violet-500 hover:text-violet-700 border border-zinc-100 hover:border-violet-200 px-3 py-2 rounded-xl transition-all cursor-pointer"
                    >
                      {s}
                    </motion.div>
                  ))}
        ...
        <div className="flex items-center gap-2 bg-zinc-100 rounded-2xl pl-3 pr-1.5 py-1.5">
          {messages.length > 0 && (
            <motion.button
              onClick={() => getAISuggestions()}
              whileTap={{ scale: 0.9 }}
              className={`shrink-0 w-8 h-8 rounded-xl flex items-center justify-center cursor-pointer transition-all ${showAI ? "bg-violet-600 text-white" : "bg-white text-violet-500 hover:bg-violet-50 border border-zinc-200"}`}
            >
              <Sparkles size={14} />
            </motion.button>
          )}
          ...
          <motion.button
            ...
            onClick={sendMsg}
            disabled={!text.trim()}
            className={`shring-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all ${text.trim() ? "bg-zinc-950 text-white hover:bg-zinc-800 cursor-pointer" : "bg-transparent text-zinc-300 cursor-not-allowed"}`}
          >
            <Send size={14} />
          </motion.button>
        </div>
